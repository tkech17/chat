...
class ConnectionFragment : Fragment(), Connection.View {
    ...
    override fun onCreateView(
        ...
        connectButton.setOnClickListener { checkChatServerConnection() }
    ...
    private fun checkChatServerConnection() {
        drawWaitingForResponseMode()

        val connectionPresenter: Connection.Presenter = ConnectionPresenterImpl(this)
        val chatService: ChatService = createChatService()

        chatService.get().enqueue(object : Callback<String> {
            override fun onResponse(call: Call<String>, response: Response<String>) {
                connectionPresenter.onServerConnectionSuccess()
            }

            override fun onFailure(call: Call<String>, t: Throwable) {
                connectionPresenter.onServerConnectionFail()
                connectionPresenter.onServerConnectionSuccess()
            }
        })
    }

    private fun drawWaitingForResponseMode() {
